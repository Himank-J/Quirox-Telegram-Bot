...
class GmailService:
    ...
    async def send_email(self, recipient_id: str, subject: str, memory: List[dict]) -> dict:
        ...
        try:
            ...
            for record in memory:
                ...
                appointment_details = interaction.get("appointment_details", "")
                if appointment_details:
                    patient_name = appointment_details.get("patient_name", "")
                    patient_age = appointment_details.get("patient_age", "")
                    patient_gender = appointment_details.get("patient_gender", "")
                    patient_symptoms = appointment_details.get("patient_symptoms", "")
                    patient_location = appointment_details.get("patient_location", "")
                    preferred_time_slot = appointment_details.get("preferred_time_slot", "")
                    ...
                    message += f"Patient age: {patient_age}\n"
                    message += f"Patient gender: {patient_gender}\n"
                    message += f"Patient symptoms: {patient_symptoms}\n"
                    message += f"Patient location: {patient_location}\n"
                    message += f"Preferred time slot: {preferred_time_slot}\n"
                    message += f"Doctor name: {doctor_name}\n"
                    message += f"Clinic name: {clinic_name}\n"
                    message += f"Appointment date: {appointment_date}\n"
